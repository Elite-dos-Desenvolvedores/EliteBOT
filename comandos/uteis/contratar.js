...
const xx = client.emojis.find("name", "xx")
    ...
    message.author.send(embed).catch(err=> message.channel.send(`${message.author}, não consegui enviar mensagem a você, ative sua DM e tente novamente.`)).then(async msg=> {
        ...
        var collector =  message.author.dmChannel.createMessageCollector(b => b.author.id == message.author.id,{time: 1000 * 50,max: 1})
...
`)
            .setFooter("© EdD", message.author.avatarURL)
            msg.edit(embedb)
            var collector =  message.author.dmChannel.createMessageCollector(c => c.author.id == message.author.id,{time: 1000 * 50,max: 1})
...
\`💡\` Resumo do projetos: ${resumo}`)
        ...
        var collector =  message.author.dmChannel.createMessageCollector(d => d.author.id == message.author.id,{time: 1000 * 50,max: 1})
...
\`📪\` Salário mensal: ${valor}`)
        ...
        msg.edit(embedd).then(async r=>{
            ...
            let sL = msg.createReactionCollector(s, {time: 120000})
            let nL = msg.createReactionCollector(n, {time: 120000})
